...
do
    p=$(cat "$t")
    i=$(basename "$t")
    ./karp-rabin --text "$dir"/chrI.fa.gz --pattern="$p" --rounds=5 | grep -oP "\d+"| sort -n > "${dir}/output/$i".1
    ./karp-rabin --text "$dir"/chr1.fa.gz --pattern="$p" --rounds=5 | grep -oP "\d+"| sort -n > "${dir}/output/$i".2
done
