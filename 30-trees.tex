...
\end{itemize}
\end{block}
\onslide<2->{\begin{block}{Unificazione}
$B(x)$: insieme degli stati $z$ tali che $P[x,z]$ è minimo.
\onslide<3->{ \alert{$B(x) = S(x)$}}
\end{block}}
\onslide<4>{Come estendere Fitch ad albero generico (sempre caso non pesato)?}
\end{frame}
...
\begin{frame}
        \frametitle{Alberi evolutivi --- Distanze}
...
\begin{frame}
\frametitle{Distanze additive: Esempio}
\begin{columns}
\begin{column}{0.48\textwidth}
{
  \begin{tabular}{l|cccccc}
...
\begin{tikzpicture}[
    ...
    cnode/.style={circle, draw, minimum size=0.5cm, inner sep=0pt},
    snode/.style={rectangle, draw, minimum size=0.6cm, inner sep=0pt, thick}
]

% 1. Bottom Row: Square Nodes (Leaves)
\node[snode, label=below:A] (A) at (0, 0) {};
\node[snode, label=below:D] (B) at (1.5, 0) {};
\node[snode, label=below:C] (C) at (3, 0) {};
\node[snode, label=below:B] (D) at (4.5, 0) {};
\node[snode, label=below:E] (E) at (6, 0) {};

% 2. Middle Row: Circle Nodes
% N1 is centered above A and B
\node[cnode] (N1) at (0.75, 2) {};
% N2 is directly above C
\node[cnode] (N2) at (3, 3) {};

% 3. Top Row: Root Node
% R is centered above D
\node[cnode] (R) at (4.5, 4) {};

% 4. Draw Edges with Labels
% From Left Circle (N1)
\draw (N1) -- (A) node[midway, left] {1};
\draw (N1) -- (B) node[midway, right] {3};

% Horizontal connection between circles
\draw (N1) -- (N2) node[midway, above] {4};

% From Middle Circle (N2)
\draw (N2) -- (C) node[midway, left] {2};

% From Root (R)
\draw (R) -- (N2) node[midway, above left] {2};
\draw (R) -- (D) node[midway, left] {1};
\draw (R) -- (E) node[midway, right] {3};
